...
def _get_frames(tback, is_debug):
    ...
    while tback is not None:
        if tback.tb_next == None and is_debug:
            break
...
def _get_response_output(handler):
    if handler.response.body:
        response_output = handler.response.body
    else:
        response_output = ''
...
def _get_response_headers(handler):
    return [(k, v) for (k, v) in handler.response.headers.iteritems()]
...
def render_html(handler):
    ...
    params = {
        ...
        'request_cookies': handler.request.cookies,
    ...
    return handler.template_response('error_template.html', **params)
...
def render_verbose_txt(handler):
    ...
    for frame in frames:
        ...
        if frame.context_line:
            _writeln(buffer, '    ' + frame.context_line.strip())
        if frame.vars:
            _writeln(buffer, dicttable_txt(frame.vars, 6))
