...
class AuthMiddleware(object):
    ...
    def _load_strategy(self, provider):
        try:
            strategy_location = self._config['provider.%s' % provider]['class_path']
            return utils.import_class(strategy_location)
        except Exception, e:
            logging.exception(e)
            raise Exception("You must provide a location for the %s strategy. Add a 'location' key to the 'provider.%s' config dict" % (provider, provider))
